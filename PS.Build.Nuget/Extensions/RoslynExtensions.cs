...
namespace PS.Build.Nuget.Extensions
{
    internal static class RoslynExtensions
    {
        ...
        public static T GetAttribute<T>(this IAssemblySymbol symbol) where T : Attribute
        {
            return symbol.GetAttributes<T>().FirstOrDefault();
        }

        public static IEnumerable<T> GetAttributes<T>(this IAssemblySymbol symbol) where T : Attribute
        {
            return symbol.GetAttributes().Where(a => a.AttributeClass.IsEqual(typeof(T))).Select(a => a.CreateAttribute()).OfType<T>();
        }

        public static bool IsEqual(this ISymbol symbol, Type type)
        {
            if (symbol == null) throw new ArgumentNullException(nameof(symbol));
            if (type == null) throw new ArgumentNullException(nameof(type));
            if (symbol.ContainingAssembly == null) return false;
            var containingType = symbol.ContainingType ?? (symbol as INamedTypeSymbol)?.ConstructedFrom;
            if (containingType == null) return false;

            var symbolAssemblyQualifiedName = string.Join(", ",
                                                          containingType.ToDisplayString(),
                                                          symbol.ContainingAssembly.ToDisplayString());

            return Equals(symbolAssemblyQualifiedName, type.AssemblyQualifiedName);
        }
